...
planet = sphere(pos=vector(50, 0, 0), color=color.blue,
                radius=1, make_trail=True)
star = sphere(pos=vector(1e-5, 0, 0), color=color.yellow,
              radius=3, make_trail=True)
planet.mass = 100
star.mass = 10e8
...
while True:
    ...
    print(planet.pos)
    planet_a = ((planet.pos-star.pos).hat*(g_acceleration(star.mass,
                planet_r)-centrifugal_acceleration(planet.velocity, planet_r)))
